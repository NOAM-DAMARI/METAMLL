...
if uploaded_file or video_url:
    with st.spinner("⏳ מעבד את הקובץ..."):
        ...
        if translate_to != "ללא":
            transcribed_text = GoogleTranslator(source="auto", target=lang_code).translate(transcribed_text)
        ...
        # יצירת קובצי ייצוא
        files = {
            "transcription.txt": transcribed_text.encode(),
            "transcription.json": json.dumps({"text": transcribed_text}, ensure_ascii=False).encode()
        }
        ...
        with col2:
            ...
            st.download_button("📥 הורד הכול כ-ZIP", open(zip_buffer.name, "rb"), "transcription.zip", "application/zip")
